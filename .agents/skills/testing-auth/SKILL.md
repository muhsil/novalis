# Testing Novalis Authentication
...
3. Verify password hash via WooCommerce API: `GET /wp-json/wc/v3/customers?email=<email>` — check `novalis_password` meta key matches PBKDF2 regex `^[0-9a-f]{32}:[0-9a-f]{128}$`
...
- The password hash is stored under `novalis_password` (no underscore prefix) so it's readable via API
- If login fails with "Please use the password you registered with", the meta key may have been changed back to `_novalis_password`
