    ...
    <>
      ...
      <footer className="bg-[#742938] max-md:hidden text-[#F9F7F2] py-20">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 border-t border-[#F9F7F2]/10 pt-16">
            ...
            <div>
              <Link href="/" className="inline-block mb-6">
                <img src="/novalis-logo.png" alt="Novalis" className="h-10 w-auto brightness-0 invert" />
              </Link>
              <p className="text-[#F9F7F2]/60 text-[13px] leading-relaxed mb-6 font-light">
                {t(locale, 'footer.brand_description')}
              </p>
              <div className="flex gap-4">
                {SOCIAL_LINKS.map((s) => (
                  <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" className="w-9 h-9 border border-[#F9F7F2]/20 rounded-full flex items-center justify-center hover:bg-[#D4AFB9] hover:border-[#D4AFB9] transition-all duration-300 transform hover:-translate-y-1" aria-label={s.label}>
                    {s.icon}
                  </a>
                ))}
              </div>
            ...
            {/* Shop */}
            <div>
              <h4 className="text-[#D4AFB9] text-[11px] font-bold mb-6 uppercase tracking-[0.2em]">{t(locale, 'footer.shop')}</h4>
              <div className="flex flex-col gap-3">
                {SHOP_LINKS.map(link => (
                  <Link key={link.href} href={link.href} className="text-[#F9F7F2]/60 text-[13px] hover:text-[#D4AFB9] transition-colors font-light">
            ...
            {/* Company */}
            <div>
              <h4 className="text-[#D4AFB9] text-[11px] font-bold mb-6 uppercase tracking-[0.2em]">{t(locale, 'footer.company')}</h4>
              <div className="flex flex-col gap-3">
            ...
            {/* Contact */}
            <div>
              <h4 className="text-[#D4AFB9] text-[11px] font-bold mb-6 uppercase tracking-[0.2em]">{t(locale, 'footer.contact_us')}</h4>
              <div className="flex flex-col gap-4 text-[#F9F7F2]/60 text-[13px] font-light">
                <span className="flex items-start gap-3">
                  <svg className="w-5 h-5 text-[#D4AFB9] shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                  {t(locale, 'nav.dubai_uae')}
                </span>
                <a href="https://wa.me/971563554303" className="hover:text-[#D4AFB9] transition-colors flex items-center gap-3">
                  <svg className="w-5 h-5 text-[#D4AFB9] shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" /></svg>
                  +971 56 355 4303
                </a>
                <a href="mailto:info@novalis.ae" className="hover:text-[#D4AFB9] transition-colors flex items-center gap-3">
                  <svg className="w-5 h-5 text-[#D4AFB9] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                  info@novalis.ae
                </a>
              </div>
            </div>
          ...
          {/* Bottom bar */}
          <div className="mt-16 pt-8 border-t border-[#F9F7F2]/10 flex items-center justify-between flex-wrap gap-8">
            <p className="text-[#F9F7F2]/30 text-xs">{t(locale, 'footer.copyright', { year: String(year) })}</p>
            <PaymentIcons />
            <p className="text-[#F9F7F2]/30 text-xs tracking-widest uppercase">{t(locale, 'footer.tagline')}</p>
          </div>
        </div>
      </footer>
