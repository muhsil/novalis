...
export const metadata: Metadata = {
  title: 'Contact Us',
  description: 'Get in touch with Novalis. We\'re here to help with your perfume orders, questions, and fragrance consultations.',
  alternates: { canonical: '/contact' },
};

const CONTACT_ICONS: Record<string, React.ReactNode> = {
  email: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>,
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 max-md:px-3 py-8 max-md:py-5 max-md:pb-20">
        ...
        {CONTACT_METHODS.map((m) => (
          <div key={m.label} className="bg-white rounded-lg border border-[#f0f0f0] p-4 flex items-start gap-3">
            ...
            <div>
              ...
              {m.href ? (
                <a href={m.href} className="text-sm text-[#C9A96E] hover:underline" target={m.href.startsWith('http') ? '_blank' : undefined} rel={m.href.startsWith('http') ? 'noopener noreferrer' : undefined}>
                  {m.value}
                </a>
              ) : (
      ...
      <div className="bg-[#FAF6F0] rounded-lg p-6 max-md:p-4 text-center">
        <h2 className="text-lg font-bold text-[#C9A96E] mb-2">Need Help Quickly?</h2>
